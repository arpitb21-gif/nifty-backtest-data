...
To keep every strategy tested on an identical, comparable period, all 14
...
Only **one of the 14 Phase 1 tasks** needs minute-level data:
**Expiry-Day Pinning** (Options Task, checks max-pain distance at a specific
intraday time). Every other strategy, control, and the benchmark run
entirely on daily data (items 1–4 above).
...
- Bid-ask spread is not present in any daily source (only in Kite's live
  ...
  empirically-derived spread model used in place of this.
